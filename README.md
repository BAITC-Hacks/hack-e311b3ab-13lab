...
| `compose.yaml` | Полный стек: приложение, PostgreSQL, MinIO |
...
Аудио, транскрипты и результаты находятся в `data/`, исключённом из Git и Docker build context. Ключи не передаются браузеру. Вывод модели отображается через `textContent`, а не вставляется как HTML. Имя файла генерируется сервером. Перенаправления провайдера отключены. Персональные учётные записи и роли работают в одном рабочем пространстве; разделения организаций и SSO пока нет.
