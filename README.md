...
В `.env` установите `TILQAZYNA_API_KEY`, а также `ADMIN_EMAIL` и `ADMIN_PASSWORD` для первого администратора. Не добавляйте ключи в исходники. Затем:
...
При первом запуске, если активного администратора нет, он создаётся из `ADMIN_EMAIL` / `ADMIN_PASSWORD` (пароль не короче 10 символов). После этого переменные можно убрать. Альтернатива без переменных окружения:

```bash
python -m scripts.create_user --email admin@example.kz --name "Администратор" --role admin
```

Администратор создаёт остальных пользователей в API `/api/users`.
