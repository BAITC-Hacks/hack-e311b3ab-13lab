...
**Статус: рабочий MVP, не завершённая промышленная система.** Распознавание выполняется через настроенный сервис Til-Qazyna; извлечение и проверка полей — настроенной LLM. Диаризация подключается отдельно через локальную модель или сервис на GPU. Неопределённые имена, исполнители и сроки требуют проверки человеком; точность на казахской и смешанной речи ещё нужно измерить на размеченном наборе. Результаты и ограничения интеграционной проверки: [docs/INTEGRATION_VALIDATION.md](docs/INTEGRATION_VALIDATION.md), [docs/OWNER_SUMMARY_FOLLOWUP.md](docs/OWNER_SUMMARY_FOLLOWUP.md).
...
✅ реализовано и проверено · 🟡 реализовано частично или без полной проверки · ⏳ не реализовано
...
| Подключение участником Teams / Zoom / Google Meet | 🟡 | Бот «HATTAMA.AI Секретарь» входит в веб-клиент по ссылке. Teams: гостевой вход без аккаунта реализован и проходит до зала ожидания; Zoom: вход из браузера реализован, на реальной встрече не проверен; Google Meet: нужен аккаунт Google для бота |
...
| 2. Напоминание при приближении срока и просрочке | 🟡 | Просроченные поручения выделяются в протоколе и в «Моих поручениях»; автоматическая рассылка напоминаний ⏳ |

### Дополнительно

| Пункт ТЗ | Статус | Комментарий |
| --- | --- | --- |
| Развёртывание в закрытом контуре (on-premise) | ✅ | Весь стек в `docker compose`: frontend, backend, PostgreSQL, MinIO, бот. Распознавание и LLM — self-hosted Til-Qazyna; внешние облачные ИИ-API не используются |
| Дашборд статусов поручений | 🟡 | Статусы «Открыто / В работе / Выполнено», просрочка, статистика по совещанию, «Мои поручения», аналитика админ-панели |
| Интеграция с СЭД | ⏳ | Точка интеграции есть: REST API и JSON-выгрузка протокола |
| Рассылка выдержки ответственным | ⏳ | Исполнитель видит свои поручения после утверждения; рассылки нет |
| Классификация поручений по срочности/направлению | ⏳ | |
| Голосовая идентификация по тембру | ⏳ | Диаризация различает голоса; ФИО назначает секретарь |
...
Развитие: автоматическая рассылка напоминаний и выдержек, интеграция с СЭД, классификация поручений, голосовая идентификация по тембру, метрики WER и DER на размеченном наборе.
