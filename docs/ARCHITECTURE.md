...
| --- | --- |
| `app/main.py` | API, загрузка, авторизация, жизненный цикл и очередь |
| `app/config.py` | Конфигурация окружения |
| `app/models.py` | Валидируемые структуры данных |
| `app/store.py` | Транзакции SQLite, версии документов |
| `app/pipeline.py` | ASR, сегментация, адаптер диаризации, извлечение и проверка |
| `app/deadlines.py` | Консервативное разрешение дат относительно даты встречи |
| `app/exports.py` | Экспорт утверждаемого черновика |
| `app/static/` | Интерфейс без внешних JS/CDN зависимостей |
| `scripts/smoke.py` | Воспроизводимая проверка реального сервиса |

## Состояния и сбои

`queued → transcribing → diarizing → analyzing → ready`; ошибка любого этапа переводит встречу в `failed`. Транскрипт сохраняется до обращения к LLM, поэтому ошибка анализа не уничтожает распознанный текст. Ручной повтор сейчас запускает весь конвейер, в том числе ASR. Автоматических повторов запросов нет, чтобы не создавать неограниченную нагрузку.

Веб-очередь ограничена пятью активными/ожидающими заданиями, одновременно выполняется одно. Это MVP, а не распределённая очередь. SQLite включён в WAL; каждое соединение закрывается. Изменение документа использует `BEGIN IMMEDIATE` и проверку версии, чтобы устаревший редактор не перезаписал новые правки. При запуске приложения незавершённые записи отмечаются прерванными. Несколько процессов на одной базе не поддерживаются.
...
Для production: отдельный worker и долговечная очередь, PostgreSQL с нормализованными поручениями и историей, SSO/RBAC, защищённое хранилище с retention, локальные версии моделей с фиксацией хешей, мониторинг латентности и качества. Отправку уведомлений строить через transactional outbox с дедупликацией и разрешением человека, затем подключать адаптер СЭД.
